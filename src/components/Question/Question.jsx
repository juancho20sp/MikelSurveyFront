...
const Question = ({
  ...
  const isSelectMissing =
    Object.keys(errors).filter((key) => key === selectId).length > 0;

  return (
    <>
      <div
        style={is_shared ? divStyle : divStyleSingleQuestion}
      ...
      >
        ...
        {!is_shared &&
          (idQuestion === 0 ? (
            ...
            <select
              name='selectRanks'
              style={isSelectMissing ? selectStylesDanger : selectStyles}
              className='rounded-lg'
              {...register(selectId, { required: true })}
            >
        ...
        {is_shared &&
          topics
            .filter((topic) => topic.title !== 'SHARED')
            .map((option) => (
              <Option
                key={option.id}
                title={option.title}
        ...
        {Object.keys(errors).filter((key) =>
          key.includes(`question-${idQuestion}-`)
        ).length > 0 && <p style={errorTag}>Esta pregunta es obligatoria</p>}
      </div>
    </>
  );
};
